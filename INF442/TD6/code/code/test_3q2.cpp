...
int main(int argc, char* argv[]) {
	...
	assert(projection.getOriginalDimension() == train_dataset.getDim() - 1);
    std::cout << "Success!" << std::endl;
	std::cout << "Classification column is OK?" << std::endl;
	assert(projection.getColClass() == col_class);
    std::cout << "Success!" << std::endl;
	std::cout << "Projection dimension is OK?" << std::endl;
	assert(projection.getProjectionDim() == projection_dim);
    std::cout << "Success!" << std::endl;
	std::cout << "Type of sampling is OK?" << std::endl;
	assert(projection.getTypeSample() == sampling);
    std::cout << "Success!" << std::endl;

	// Tests for projection matrix
	std::cout << "Has d rows?" << std::endl;
    assert(projection.getProjection().rows() == train_dataset.getDim()- 1);
    std::cout << "Success!" << std::endl;
    std::cout << "Has l columns?" << std::endl;
    assert(projection.getProjection().cols() == projection_dim);
    std::cout << "Success!" << std::endl;
